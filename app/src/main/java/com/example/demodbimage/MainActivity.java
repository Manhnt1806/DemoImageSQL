...
import com.example.demodbimage.DbHelper;
...
//                db.Insertsanpham(edtten.getText().toString(), Integer.parseInt(edtgia.getText().toString()),ConverttoArrayByte(imganh));
                ...
                byte[] anh = ConverttoArrayByte(imganh);
                SanPham kc = new SanPham(tenSP, gia, anh);

                if (dao.insertRow(kc)) {
                    Toast.makeText(MainActivity.this, "Thêm khoản thu thành công", Toast.LENGTH_SHORT).show();
//                    list.clear();
//                    list.addAll(kcDAO.getAll());
//                    adapter.notifyDataSetChanged();
                } else {
                    Toast.makeText(MainActivity.this, "Thêm khoản thu thất bại", Toast.LENGTH_SHORT).show();
                }
